...
public class UIDrawCallTest : MonoBehaviour
{
    static BetterList<UIDrawCallTest> mActiveList = new BetterList<UIDrawCallTest>();
    static BetterList<UIDrawCallTest> mInactiveList = new BetterList<UIDrawCallTest>();

    [System.Obsolete("Use UIDrawCallTest.activeList")]
    static public BetterList<UIDrawCallTest> list { get { return mActiveList; } }

    /// <summary>
    /// List of active draw calls.
    /// </summary>

    static public BetterList<UIDrawCallTest> activeList { get { return mActiveList; } }

    /// <summary>
    /// List of inactive draw calls. Only used at run-time in order to avoid object creation/destruction.
    /// </summary>

    static public BetterList<UIDrawCallTest> inactiveList { get { return mInactiveList; } }
    ...
    public Texture mTexture;		// Main texture used by the material
    ...
    [HideInInspector]
    public int[] mIndices;		// Cached indices
    ...
    void UpdateMaterials()
    {
        ...
        else if (mRenderer && mRenderer.sharedMaterial != mDynamicMat)
        {
#if UNITY_EDITOR
    ...
    static public UIDrawCallTest Create(UIPanel panel, Material mat, Texture tex, Shader shader)
    {
#if UNITY_EDITOR
        string name = null;
    ...
    static UIDrawCallTest Create(string name, UIPanel pan, Material mat, Texture tex, Shader shader)
    {
        UIDrawCallTest dc = Create(name);
    ...
    static UIDrawCallTest Create(string name)
    {
#if SHOW_HIDDEN_OBJECTS && UNITY_EDITOR
		name = (name != null) ? "_UIDrawCallTest [" + name + "]" : "DrawCall";
#endif
        while (mInactiveList.size > 0)
        {
            UIDrawCallTest dc = mInactiveList.Pop();
		...
		HideFlags.DontSave | HideFlags.NotEditable, typeof(UIDrawCallTest));
#else
 HideFlags.HideAndDontSave, typeof(UIDrawCallTest));
#endif
        UIDrawCallTest newDC = go.GetComponent<UIDrawCallTest>();
#else
		GameObject go = new GameObject(name);
		DontDestroyOnLoad(go);
		UIDrawCallTest newDC = go.AddComponent<UIDrawCallTest>();
#endif
    ...
    static public void ClearAll()
    {
        ...
        {
            UIDrawCallTest dc = mActiveList[--i];
    ...
    static public void ReleaseInactive()
    {
        ...
        {
            UIDrawCallTest dc = mInactiveList[--i];
    ...
    static public void Destroy(UIDrawCallTest dc)
    {
        if (dc)
        {
            dc.onRender = null;
